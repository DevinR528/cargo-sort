...
impl Index for String {
    fn index<'v>(&self, v: &'v Item) -> Option<&'v Item> { self[..].index(v) }
    fn index_or_insert<'v>(&self, v: &'v mut Item) -> &'v mut Item {
...
where
    I: Index,
{
    fn index_mut(&mut self, index: I) -> &mut Item { index.index_or_insert(self) }
}
...
impl<'s> ops::Index<&'s str> for Document {
    ...
    fn index(&self, key: &'s str) -> &Item { self.root.index(key) }
}

impl<'s> ops::IndexMut<&'s str> for Document {
    fn index_mut(&mut self, key: &'s str) -> &mut Item { self.root.index_mut(key) }
}
