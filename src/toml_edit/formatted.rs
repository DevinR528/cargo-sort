...
impl From<InternalString> for Value {
    fn from(s: InternalString) -> Self { Value::from(s.as_ref()) }
}
...
impl From<Array> for Value {
    fn from(array: Array) -> Self { Value::Array(array) }
}

impl From<InlineTable> for Value {
    fn from(table: InlineTable) -> Self { Value::InlineTable(table) }
}
...
impl<'k, K: Into<&'k Key>, V: Into<Value>> FromIterator<(K, V)> for Value {
    ...
    {
        let mut table =
            InlineTable { items: to_key_value_pairs(iter), ..Default::default() };
